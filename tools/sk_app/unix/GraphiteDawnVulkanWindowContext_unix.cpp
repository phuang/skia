...
GraphiteDawnVulkanWindowContext_unix::GraphiteDawnVulkanWindowContext_unix(const XlibWindowInfo& info,
...
bool GraphiteDawnVulkanWindowContext_unix::onInitializeContext() {
    ...
    auto device = createDevice(wgpu::BackendType::Vulkan);
    if (!device) {
        SkASSERT(device);
        return false;
    }
...
wgpu::SwapChain GraphiteDawnVulkanWindowContext_unix::createSwapChain() {
