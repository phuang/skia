...
protected:
    ...
    virtual sk_sp<SkTypeface> onLegacyMakeTypeface(const char familyName[],
                                                   SkFontStyle style) const override;

private:
    sk_sp<FontConfig_OHOS> fontConfig;   // the pointer of FontConfig_OHOS
...
SkFontMgr_OHOS::SkFontMgr_OHOS(const char* path) {
    fontConfig = sk_make_sp<FontConfig_OHOS>(&fontScanner, path);
...
sk_sp<SkTypeface> SkFontMgr_OHOS::onMatchFamilyStyle(const char familyName[],
    ...
    return fontConfig->getTypeface(styleIndex, style, isFallback);
}
...
sk_sp<SkTypeface> SkFontMgr_OHOS::onMatchFamilyStyleCharacter(const char familyName[],
    ...
    while (true) {
        if (bcp47Count > 0) {
            auto retTp = findTypeface(*item, style, bcp47, bcp47Count, character);
            if (retTp) {
                return retTp;
            }
...
sk_sp<SkTypeface> SkFontMgr_OHOS::findTypeface(const FallbackSetPos& fallbackItem,
    ...
    for (unsigned int i = fallbackItem.index;
         ...
         i++) {
        int ret = compareLangs(fallbackSet[i]->langs, bcp47, bcp47Count, tps.data());
        if (ret == -1) {
            continue;
        }
        tps[ret] = i;
 ...
 * \note The caller must call unref() on the returned object if it's not null
 */
sk_sp<SkTypeface> SkFontMgr_OHOS::onMatchFaceStyle(const SkTypeface* typeface,
                                                   const SkFontStyle& style) const {
    if (typeface == nullptr) {
        return nullptr;
    }
