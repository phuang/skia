...
static const char* PRODUCT_DEFAULT_CONFIG = "/system/etc/fontconfig.json";
 ...
 */
sk_sp<SkTypeface> FontConfig_OHOS::getTypeface(int styleIndex, int index, bool isFallback) const {
 ...
 */
sk_sp<SkTypeface> FontConfig_OHOS::getTypeface(int styleIndex,
...
int FontConfig_OHOS::getStyleIndex(const char* familyName, bool& isFallback) const {
    ...
    auto* p = genericNames.find(fname);
    if (p) {
        isFallback = false;
        return *p;
    } else {
        ...
        auto* p = fallbackNames.find(fname);
        if (p) {
            isFallback = true;
            return *p;
        }
...
int FontConfig_OHOS::parseConfig(const char* fname) {
    ...
    while (true) {
        ...
        if (!root.isMember(key)) {
            ...
            return logErrInfo(
                    ERROR_CONFIG_INVALID_VALUE_TYPE, key, Json::arrayValue, root[key].type());
        }
        ...
        for (unsigned int i = 0; i < arr.size(); i++) {
            if (arr[i].isObject()) {
                ...
                (void)logErrInfo(ERROR_CONFIG_INVALID_VALUE_TYPE,
                                 errKey.c_str(),
                                 Json::objectValue,
                                 arr[i].type());
            }
...
void FontConfig_OHOS::dumpGeneric() const {
    ...
    for (unsigned int i = 0; i < genericFamilySet.size(); i++) {
        LOGI("[%d] familyName : %s - %d\n",
             i,
             genericFamilySet[i]->familyName.c_str(),
...
int FontConfig_OHOS::parseFontDir(const char* fname, const Json::Value& root) {
    for (unsigned int i = 0; i < root.size(); i++) {
        ...
        defined(SK_BUILD_FONT_MGR_FOR_PREVIEW_LINUX)
            ...
            return logErrInfo(ERROR_CONFIG_INVALID_VALUE_TYPE,
                              text.c_str(),
                              Json::stringValue,
                              root[i].type());
        }
...
int FontConfig_OHOS::parseGeneric(const Json::Value& root) {
    ...
    if (!root.isMember(key)) {
        ...
        return logErrInfo(
                ERROR_CONFIG_INVALID_VALUE_TYPE, key, Json::stringValue, root[key].type());
    }
...
int FontConfig_OHOS::parseAdjust(const Json::Value& root, std::vector<AdjustInfo>& adjustSet) {
    ...
    for (unsigned int i = 0; i < sizeof(tags) / sizeof(char*); i++) {
        ...
        if (!root.isMember(key)) {
            ...
            return logErrInfo(
                    ERROR_CONFIG_INVALID_VALUE_TYPE, key, Json::intValue, root[key].type());
        } else {
 ...
 */
int FontConfig_OHOS::parseVariation(const Json::Value& root,
                                    std::vector<VariationInfo>& variationSet) {
    const char* key = nullptr;
    ...
    for (unsigned int i = 0; i < sizeof(tags) / sizeof(char*); i++) {
        ...
        if (!strcmp(key, "weight")) {
            ...
            if (!root[key].isNumeric()) {
                return logErrInfo(
                        ERROR_CONFIG_INVALID_VALUE_TYPE, key, Json::realValue, root[key].type());
            }
            Coordinate axis;
...
bool FontConfig_OHOS::insertTtcFont(int count, FontInfo& font) {
    bool ret = false;
    ttcIndexMap.foreach (
            [this, count, &font, &ret](const SkString& familyName, TtcIndexInfo* info) {
                if (info->familyName == font.familyName && info->ttcIndex < count) {
                    ...
                    if (tpSet) {
                        ...
                        sk_sp<SkTypeface_OHOS> typeface =
                                sk_make_sp<SkTypeface_OHOS>(specifiedName, newFont);
...
int FontConfig_OHOS::loadFont(const SkFontScanner* scanner, const char* fname) {
    ...
    if (adjustMap.find(font.familyName) != nullptr) {
        const std::vector<AdjustInfo> adjustSet = adjustMap[font.familyName];
        for (unsigned int i = 0; i < adjustSet.size(); i++) {
            if (font.style.weight() == adjustSet[i].origValue) {
                font.style =
                        SkFontStyle(adjustSet[i].newValue, font.style.width(), font.style.slant());
                break;
            }
        }
    }
...
int FontConfig_OHOS::scanFonts(const SkFontScanner* fontScanner) {
    ...
    for (unsigned int i = 0; i < fontDirSet.size(); i++) {
        ...
        while ((node = readdir(dir))) {
            ...
            std::vector<char> fullname(len, 0);
            strncpy(fullname.data(), fontDirSet[i].c_str(), len);
#if defined(SK_BUILD_FONT_MGR_FOR_PREVIEW_WIN)
...
#endif
            strncat(fullname.data(), fname, len);
            loadFont(fontScanner, fullname.data());
        }
        closedir(dir);
    }
    fontDirSet.clear();
 ...
 * \return err
 */
int FontConfig_OHOS::logErrInfo(int err,
                                const char* key,
