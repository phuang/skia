 ...
 */
struct FallbackSetPos {
    unsigned int index;  // the index of the first font style set in the fallback set for a
 ...
 */
class FontConfig_OHOS : public SkRefCntBase {
public:
    explicit FontConfig_OHOS(const SkFontScanner* fontScanner, const char* fname = nullptr);
    ...
    sk_sp<SkTypeface> getTypeface(int styleIndex, int index, bool isFallback = false) const;
    sk_sp<SkTypeface> getTypeface(int styleIndex,
    ...
    static sk_sp<SkTypeface_OHOS> matchFontStyle(const TypefaceSet& typefaceSet,
