...
SpvId SPIRVCodeGenerator::getFunctionParameterType(const Type& parameterType) {
    ...
    //
    // We also disallow unified sampler types from being passed as a function parameter in
    // user-defined functions as this is not supported by the WGSL specification.
    if (parameterType.typeKind() == Type::TypeKind::kSampler) {
        SkDEBUGFAILF("unsupported function parameter type: %s",
                     parameterType.description().c_str());
        return NA;
    }
    SpvStorageClass_ storageClass;
    if (parameterType.typeKind() == Type::TypeKind::kSeparateSampler ||
...
SpvId SPIRVCodeGenerator::writeFunctionCallArgument(const FunctionCall& call,
    ...
    if (is_out(paramModifiers)) {
        ...
        // As noted in getFunctionParameterType, opaque handle (sampler/texture) arguments are
        // always declared as pointers but never stored in intermediates when calling user-defined
        // functions.
        //
        // The case for intrinsics (which opaque handle arguments by value) is handled above just
...
static SpvStorageClass_ get_storage_class_for_variable(const Variable& var,
                                                       SpvStorageClass_ fallbackStorageClass) {
    const Modifiers& modifiers = var.modifiers();
    if ((modifiers.fFlags & Modifiers::kIn_Flag) && var.storage() == Variable::Storage::kGlobal) {
        SkASSERT(!(modifiers.fLayout.fFlags & Layout::kPushConstant_Flag));
        return SpvStorageClassInput;
    }
    if ((modifiers.fFlags & Modifiers::kOut_Flag) && var.storage() == Variable::Storage::kGlobal) {
        SkASSERT(!(modifiers.fLayout.fFlags & Layout::kPushConstant_Flag));
...
static SpvStorageClass_ get_storage_class(const Expression& expr) {
    switch (expr.kind()) {
        case Expression::Kind::kVariableReference: {
            ...
            return get_storage_class_for_variable(var, SpvStorageClassPrivate);
        }
        case Expression::Kind::kFieldAccess:
...
std::unique_ptr<SPIRVCodeGenerator::LValue> SPIRVCodeGenerator::getLValue(const Expression& expr,
    ...
    switch (expr.kind()) {
        ...
        default: {
            ...
            // into a temporary (which is handled explicitly in writeVariableReference).
            SpvId result = this->nextId(nullptr);
...
SpvId SPIRVCodeGenerator::writeInterfaceBlock(const InterfaceBlock& intf, bool appendRTFlip) {
    ...
    SpvStorageClass_ storageClass =
            get_storage_class_for_variable(intfVar, SpvStorageClassFunction);
...
void SPIRVCodeGenerator::writeGlobalVar(ProgramKind kind, const VarDeclaration& varDecl) {
    ...
    SpvStorageClass_ storageClass = get_storage_class_for_variable(*var, SpvStorageClassPrivate);
